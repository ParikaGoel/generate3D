...
class Trainer:
    def __init__(self, train_list, val_list, device):
        ...
        self.device = device
        self.model = Net(3, 3).to(device)

    def loss_and_optimizer(self):
        self.criterion = losses.mse
    ...
    def train(self, epoch):
        ...
        for idx, sample in enumerate(self.dataloader_train):
            input = sample['color_grid'].to(self.device)
            target = sample['color_gt'].to(self.device)
        ...
        for epoch in range(config.num_epochs):
            ...
            if val_loss < prev_val_loss:
                ...
                torch.save(self.model.state_dict(),
                           params["network_output"] + synset_id + "/saved_models/color.pth")
...
if __name__ == '__main__':
    ...
    for f in glob.glob(params["shapenet_raytraced"] + synset_id + "/*_color_.txt"):
        model_id = f[f.rfind('/') + 1:f.rfind('.')]
        train_list.append({'synset_id': synset_id, 'model_id': model_id})

    val_list = train_list[2600:3240]
    train_list = train_list[:2600]
    ...
    train_writer_path = params["network_output"] + synset_id + "/logs/logs_color/train/"
    val_writer_path = params["network_output"] + synset_id + "/logs/logs_color/val/"
