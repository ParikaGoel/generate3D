...
def occ_to_df(occ, trunc, pred=True):
    ...
    # valid_dists = df[indices_x, indices_y, indices_z] + kernel[:,None]
    # min_dist = torch.min(valid_dists, dim=0).values
    # init_dist = torch.flatten(df[1:33,1:33,1:33])
    # df = torch.reshape(torch.min(min_dist, init_dist),(32,32,32))

    for i in range(grid_coords_vol.size(1)):
        valid_dists = df[indices_x[:,i], indices_y[:,i], indices_z[:,i]] + kernel
        df[tuple(grid_coords_vol[:, i])] = torch.min(valid_dists)
...
#     occ_file = "/home/parika/WorkingDir/complete3D/Assets_remote/shapenet-voxelized-gt/04379243/fff7f07d1c4042f8a946c24c4f9fb58e__0__.txt"
#     df_file = "/home/parika/WorkingDir/complete3D/Assets/fff7f07d1c4042f8a946c24c4f9fb58e.ply"
