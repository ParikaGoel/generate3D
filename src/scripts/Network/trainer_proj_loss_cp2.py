...
class Trainer:
    ...
    def train(self, epoch):
        ...
        proj_img_out = os.path.join(params["network_output"],"data_cp2/proj_imgs/run_%02d"%epoch)
        gt_img_out = os.path.join(params["network_output"],"data_cp2/gt_imgs/run_%02d"%epoch)
        ...
        for idx, sample in enumerate(self.dataloader_train):
            ...
            loss = losses.proj_loss2(proj_imgs, imgs_gt, self.device)
            grad_loss = torch.mul(torch.sqrt(loss), 2)

            # ===================backward + optimize====================
            proj_imgs.backward(grad_loss)
            self.optimizer.step()
